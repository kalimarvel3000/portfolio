    ...
    function sliderSlideLeft(){
      ...
      if($block.querySelector('[data-layout=slideshow]')){
        if(!$current.parentElement.previousElementSibling){
          ...
          var $prev = $sliderRoot.children[$sliderRoot.children.length-1].firstElementChild
        }else{
          var $prev = $current.parentElement.previousElementSibling.firstElementChild;
        }
      }else{
        var $prev = $current.previousElementSibling;
        if(!$prev){
...
function openModal (article_id) {
    ...
    fetch("/api/v1/articles/"+article_id+"/?template&language="+document.documentElement.getAttribute("lang"))
      .then(response => response.text())
      .then(data => {
...
function loadArticles(start_from) {
  ...
  fetch("/api/v1/articles/?block="+block.dataset.id+"&order="+orderVal+"&publication="+publicationVal+"&tag="+encodeURIComponent(tagsVal)+"&start="+start_from+"&search="+encodeURIComponent(searchVal)+"&language="+document.documentElement.getAttribute("lang"))
    .then(response => response.text())
    .then(data => {
          // this.$el.removeClass('loading')
          loading = false;
